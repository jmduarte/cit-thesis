...
This appendix presents the results of two searches for dijet
...
new gauge bosons ($\PWpr$ and $\PZpr$) with SM-like or leptophobic couplings~\cite{ref_gauge}, and Randall--Sundrum (RS) gravitons
(G)~\cite{ref_rsg}. 
...
\section{Measurement of the invariant mass spectra}
...
total jet energy. An event is rejected if either of the two leading
jets fails the jet ID criteria.

\subsection{Wide jet reconstruction and event selection}

Geometrically close jets are combined into ``wide jets'' and
used to determine the dijet mass, as in the previous CMS
...
the dijet system. The background from $t$-channel dijet events peaks at large values of $\detajj$ and is
...
of dijet data to calibrate the wide jets reconstructed from Calo jets at HLT. With this correction, based on a 
dijet balance tag-and-probe method similar to that discussed in
Ref.~\cite{Khachatryan:2016kdb}, the wide jets from Calo jets have 
the same response as those reconstructed from PF jets.
%roughly a 1\% correction.

\subsection{Dijet mass spectra and fits}
...
\begin{figure}[hbtp]
  \begin{center}
   ...
   \caption{
    Dijet mass spectra (points) compared to a fitted
  parameterization of the background (solid curve) for the low-mass search (left) and 
  the high-mass search (right)~\cite{CMS-PAS-EXO-16-032,jmgd}.
...
\begin{figure}[hbtp]
  \begin{center}
    ...
    plane (right)~\cite{CMS-PAS-EXO-16-032}. The \pt, $\eta$, and $\phi$ values of the two
...
\begin{figure}[hbtp]
  ...
  simulation of the detector~\cite{CMS-PAS-EXO-16-032}. Resonances from quark-quark processes modeled by $\PQq\PAQq\to \PXXG \to \PQq\PAQq$  (blue),
...
The procedure gives the same limits as the Bayesian procedure used previously for dijet resonance
...
signal strength $\hat{\mu}$ and its uncertainty $\sigma_{\mu}$. The resulting bias $(\hat{\mu}-\mu)/\sigma_{\mu}$
was found to be negligible as illustrated in Fig.~\ref{fig:bias}.

\begin{figure*}[hbtp]
  \centering
    \includegraphics[width=0.48\textwidth]{figs/dijet/bias_qq_1350_lumi-12p910_r-0p049_CaloDijet2016_modexp_fourparam.pdf}
    \includegraphics[width=0.48\textwidth]{figs/dijet/biasVsMass_qq_lumi-12p910_CaloDijet2016_modexp_fourparam.pdf}
    \caption{Bias $(\hat{\mu}-\mu)/\sigma_{\mu}$ when generating
      pseudodata with the alternative parameterization of
      Eqn.~\ref{eqn:modexp} and fitting with the nominal background
      parameterization of Eqn.~\ref{eqBackgroundParam} for a $\Pq\Pq$ resonance of mass $1350\GeV$ with a signal strength $\mu=0.049$
      corresponding to the 95\% CL limit on $\sigma\times B\times A$
      of  $0.49\unit{pb}$ (left). Bias for $\Pq\Pq$ resonances as a function of mass from
     $600\GeV$ to $1600\GeV$ (right)~\cite{jmgd}.
    \label{fig:bias}}
\end{figure*}
...
\begin{figure*}[hbtp]
  \centering
    ...
    $\PZpr$ with SM-like couplings~\cite{ref_gauge}, a dark matter mediator for
...
\begin{figure*}[hbtp]
  \centering
    ...
    $\PZpr$ with SM-like or leptophobic couplings~\cite{ref_gauge}, a dark matter mediator for
...
by correcting the limits. The acceptance of the minimum dijet mass requirement in each search has been evaluated separately for 
$\PQq\PQq$, $\PQq\Pg$, and $\Pg\Pg$ resonances, and has been taken into account by correcting the limits, and therefore does not appear in the acceptance $A$.
The corrections are independent of the spin and coupling of the narrow resonance at the one percent level. 
...
The difference in the limits for $\PQq\PQq$, $\PQq\Pg$, and $\Pg\Pg$, and Gaussian resonances at the same resonance mass
...
and located closer to the resonance pole than any combination of two partons can produce. 
...
\caption{95\% CL exclusion regions in
  ...
  searches~\cite{CMS-PAS-EXO-16-032,Khachatryan:2016ecr} and
  different $\MET$ based DM searches~\cite{CMS-PAS-EXO-16-030,CMS-PAS-EXO-16-037,CMS-PAS-EXO-16-039,CMS-PAS-EXO-16-010} from CMS in the leptophobic
  vector (top) and axial-vector (bottom) models~\cite{CMS-DP-2016-057,jmgd}. Following the
  recommendation of the LHC DM working
...
the limits are shown at 90\% CL~\cite{CMS-DP-2016-057,jmgd}.
...
This appendix presents two searches for narrow resonances decaying into a pair of jets, performed using
proton-proton collisions at $\sqrt{s}=13$\TeV corresponding to an
integrated luminosity of 12.9\fbinv. The first is a low-mass search
based on calorimeter jets, reconstructed by the high level trigger and recorded in compact form (data scouting),
and the second is a high-mass search based on particle-flow jets. The dijet mass spectra are observed to be smoothly falling
distributions. In the analyzed data samples, there is no evidence for resonant particle production.
Generic upper limits are presented on the product of the cross section, the branching fraction, and the acceptance for narrow quark-quark, quark-gluon, and gluon-gluon
resonances that are applicable to any model of narrow dijet resonance production.
String resonances with masses below 7.4\TeV are excluded at 95\% confidence level, as are scalar diquarks below 6.9\TeV,
axigluons and colorons below 5.5\TeV, excited quarks below 5.4\TeV, color-octet
scalars below 3.0\TeV, $\PWpr$ bosons below 2.7\TeV, $\PZpr$ bosons with SM-like couplings below 2.1\TeV and between 2.3 and 2.6\TeV,
and Randall--Sundrum gravitons below 1.9\TeV. This extends previously
published limits in the dijet channel. Finally, the first CMS limits
are set on a simplified model of dark matter mediators based on the dijet channel, excluding vector and axial-vector mediators
below 2.0\TeV for a universal quark coupling $\gq = 0.25$ and a dark matter coupling $\gDM=1.0$. Limits on the mass of a dark matter mediator are presented as a function of dark matter
mass, and are translated into upper limits on the cross section for dark matter particles scattering on nucleons
that are more sensitive than those of direct detection experiments for

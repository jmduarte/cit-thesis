...
described in Ref.~\cite{LHCCLs}.  We use a multi-bin counting experiment likelihood, which is
...
the model-independent upper limits on the cross section of $\PQq\PQq$
resonances are translated into 95\% \CLp~upper limits on the coupling
...
shows the upper limits obtained with the data scouting technique in
the mass region from 600 to 1600\GeV, which are competetive with the
coverage of previous CMS searches using data scouting at $\sqrt{s}=8
\TeV$~\cite{Khachatryan:2016ecr}.  Previous exclusions obtained with
similar searches at various collider energies are also shown. 
...
RS Graviton  & $\PQq\PAQq$, $\Pg\Pg$        & 1.9\ (1.8)    & ---            & 1.6\ (1.3)\\
 \end{tabular}
\label{tab:MassLimit}
\end{table}
...
We have presented limits with Gaussian shapes of 10\% width in order to compare the sensitivity of this search to
...
pp collisions at $\sqrt{s}=13$\TeV corresponding to an integrated luminosity of \RunLumi. A low-mass search using data scouting from the HLT trigger 
...
W$^{\prime}$ bosons, Z$^{\prime}$ bosons, and RS gravitons, which extend previously published limits in the dijet channel.

...
Analyses at $\sqrt{s}=13$\TeV have also searched for dijet resonances with masses above 1.5\TeV at CMS~\cite{Khachatryan:2015dcf}
...
DM mediator ($m_\mathrm{DM} = 1\GeV$) & $\PQq\PAQq$                       & 2.0\ (2.2)    & ---            & ---  \\
...
%\begin{equation}
%\Gamma(\Zpr_B\to jj) = 5 \frac{g_B^2m_{\Zpr_B}}{36(4\pi)}\left(1+\frac{\alpha_s}{\pi}\right)
%\end{equation}
...
$\sigma_{\mathrm{SI}}$ or $\sigma_{\mathrm{SD}}$, respectively. The SI
DM-nucleon cross section takes the form
\begin{align}
\sigma_\mathrm{SI}& = \frac{f^2(g^\prime_\Pq) g_\mathrm{DM}^2
...
\begin{align}
\sigma_\mathrm{SD} &= \frac{3f^2(g^\prime_\Pq) g_\mathrm{DM}^2
  \mu_{\Pn\chi}}{\pi m_{\mathrm{med}}^4}\\
&\simeq 2.4\times10^{-42}\unit{cm}^2 \cdot \left (\frac{g^\prime_\Pq
  g_\mathrm{DM}}{0.25} \right)^2 \left (\frac{1 \TeV}{m_\mathrm{med}}
  \right)^4 \left (\frac{\mu_{\Pn\chi}}{1\GeV} \right)^2 ~,
\end{align}
where, in general, $f(g^\prime_\Pq)$ could be different for protons
and neutrons:
\begin{equation}
f^{\Pp,\Pn}(g^\prime_\Pq) = \Delta^{(\Pp,\Pn)}_\PQu g^\prime_\PQu + \Delta^{(\Pp,\Pn)}_\PQd g^\prime_\PQd +  \Delta^{(\Pp,\Pn)}_\PQs g^\prime_\PQs~,
\end{equation} 
...
coupling $g^\prime_\Pq$ is equal for all quarks, these
factors are equal, $f^{\Pp,\Pn}(g^\prime_\Pq)  = f(g^\prime_\Pq) =
0.32 g^\prime_\Pq$. As DD experiments quote 90\% CL limits, the CMS limits are also

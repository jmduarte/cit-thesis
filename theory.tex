...
\textbf{Maria: Can you add after this section the beta functions if the self
coupling and make a small subsection on the problem of the
metastatibility of the vacuum. it involves again the looks, the top
mass and the higgs mass (and a_s I guess) and it is an important point
that Higgs is throwing us a curveball given the higgs mass and the top
mass -- I think the traditional hierarchy and the problem of the
metastability are the same problem just before and after the
measurement of the higgs.

Can point out that with SUSY included the metastability  problem is
tossed in the garbage in the same way that SUSY is the answer to the
hierarchy problem}
...
the top squark mass from the gluino:
\begin{align}
\Delta(m^2_{\PSQt_i}) &= \quad 
\parbox{20mm}{
\begin{fmfgraph*}(20,20)
...
&= \frac{2g_\mathrm{s}^2m_{\sGlu}^2}{3\pi^2}\log \left
  (\frac{\Lambda_{\mathrm{UV}}}{m_{\sGlu} \right) + \cdots~,
\end{align}
\end{fmffile}
With this, we have roughly that $m_{\sGlu} \lesssim 2 m_{\PSQt}$.

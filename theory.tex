...
results. In this scenario, the LSP is the lightest neutralino $\chiz_1$ while the NLSP
is the lightest chargino $\chipm_1$.  
%They are both higgsinos and their mass splitting is taken to be 5\GeV, motivated by
%Fig.~\ref{fig:neutralinos}. 
The NLSP is assumed to have a mass that is $5 \GeV$ larger
...
are Higgsino-like and quasi-degenerate~\cite{naturalSUSY}. 
The NLSP decays to the LSP and a virtual $\PW$ boson ($\chipm_1 \to
\PW^{\ast} \chiz_1$), whose decay products mostly have too low
momentum to identified. The specific choice of the NLSP-LSP mass
splitting is supported by Fig.~\ref{fig:neutralinos}, but does not
have a large impact on the results of the interpretation. 
 The other SUSY particles accessible at the LHC are the gluino and the

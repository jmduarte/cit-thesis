...
& +\overline\ell_\mathrm{L}\tilde\sigma^{\mu}iD_{\mu}\ell_\mathrm{L} +
   \overline e_\mathrm{R}\sigma^{\mu}iD_{\mu}e_\mathrm{R} + (\mathrm{h.c.})& \nonumber\\
&+ \overline \nu_\mathrm{R}
   \sigma^{\mu}iD_{\mu}\nu_\mathrm{R} + (\mathrm{h.c.})& \mathrm{(lepton~kinetic~and~gauge~terms)}\nonumber\\
& +\overline q_\mathrm{L}\tilde\sigma^{\mu}iD_{\mu}q_\mathrm{L} +
   \overline u_\mathrm{R}\sigma^{\mu}iD_{\mu}u_\mathrm{R}  +
  (\mathrm{h.c.}) & \nonumber\\
&  + \overline d_\mathrm{R}
...
\frac{1}{\sqrt{2}}U(x)\begin{pmatrix} 0\\v\end{pmatrix} ~,&v &= \sqrt{\frac{\mu^2}{\lambda}}~,
\end{align}
...
\begin{align}
|D_{\mu}\Phi|^2 &= \frac{1}{2} \left|\begin{pmatrix}\partial_{\mu}
    -\frac{i}{2}(g_2W_{\mu}^3 + g_1B_{\mu})&
    -\frac{ig_2}{2}(W_{\mu}^1-iW_{\mu}^2)\\ 
-\frac{ig_2}{2}(W_{\mu}^1+iW^2_{\mu})&\partial_{\mu}
    +\frac{i}{2}(g_2W_{\mu}^3 - g_1B_{\mu})
  \end{pmatrix}\begin{pmatrix}0\\v\end{pmatrix}\right |^2 \nonumber\\
...
\begin{align}
\Phi(x)&=
\frac{1}{\sqrt{2}}U(x)\begin{pmatrix} 0\\v+H(x)\end{pmatrix} 
\end{align}
...
\fmf{dashes}{i,v1}
\fmf{dashes}{v2,o}
\fmf{plain,left,tension=.3,label=$\PQt$}{v1,v2}
...
may be computed from one-loop diagrams such as those
shown in Fig~\ref{fig:lambda}:
\begin{align}
...
\fmf{fermion}{b1,v2}
\fmf{fermion}{b2,v3}
\fmf{fermion,tension=0.3,label=$\PQt$,label.side=right}{v1,v2}
\fmf{dashes,tension=0.3,label=$\PH$,label.side=right}{v2,v3}
\fmf{fermion,tension=0.3,label=$\PQt$,label.side=right}{v3,v1}
\fmfdot{v1,v2,v3}
  \end{fmfgraph*}\hfill
\begin{fmfgraph*}(50,40)
        \fmfset{arrow_len}{cm}\fmfset{arrow_ang}{0}
...
\fmflabel{$\PH$}{o2}
\fmf{dashes}{i1,v1}
\fmf{fermion,tension=0.5,label=$\PQt$,label.side=right}{v1,v2}
\fmf{dashes}{v2,o1}
\fmf{dashes}{o2,v4}
\fmf{fermion,tension=0.5,label=$\PQt$,label.side=right}{v4,v3}
\fmf{dashes}{v3,i2}
\fmf{fermion,tension=0.5,label=$\PQt$,label.side=left}{v1,v3}
\fmf{fermion,tension=0.5,label=$\PQt$,label.side=right}{v2,v4}
\fmfdot{v1,v2,v3,v4}
  \end{fmfgraph*}\hfill
\begin{fmfgraph*}(50,40)
        \fmfset{arrow_len}{cm}\fmfset{arrow_ang}{0}
...
\fmf{dashes}{i1,v1,o1}
\fmf{dashes}{o2,v2,i2}
\fmf{dashes,label=$\PH$,left,tension=0.5}{v1,v2}
\fmf{dashes,label=$\PH$,left,tension=0.5}{v2,v1}
\fmfdot{v1,v2}
  \end{fmfgraph*}
\end{fmffile}\setkeys{Gin}{width=\linewidth,totalheight=\textheight,keepaspectratio}
  \vspace{\baselineskip}
\caption{Example one-loop diagrams contributing to the running of
  $y_\PQt$ and $\lambda$~\cite{jmgd}. \label{fig:lambda}}
\end{figure}

The running of $\lambda$ is of particular interest because if it
runs to $\lambda<0$, the present electroweak vacuum of the SM
may correspond to a local minimum, or metastable ``false vacuum,'' which
will eventually decay~\cite{PhysRevD.15.2929,PhysRevD.16.1762}. Based on the
...
some $\mu$ but with a long lifetime), and instability ($\lambda<0$ for
some $\mu$ and a short lifetime). Ref.~\cite{Buttazzo:2013uya}
...
0 & \mathbf{X}^T \\
 \mathbf{X}& 0
\end{pmatrix}, ~\mathrm{where} ~ \mathbf{X} =  \begin{pmatrix}
...
parameters, respectively. The trilinear coupling parameters $\boldsymbol{a_\PQu} $,
$\boldsymbol{a_\PQd}$, and $\boldsymbol{a_\Pe}$ are a complex $3\times3$
matrices in generation space with dimensions of mass. They are in
one-to-one correspondence with the Yukawa couplings of the
superpotential. Each of $\boldsymbol{m_{\widetilde{\mathrm{Q}}}^2}$,
$\boldsymbol{m_{\widetilde{\mathrm{L}}}^2}$,
$\boldsymbol{m_{\widetilde{\mathrm{U}}^\mathrm{c}}^2}$,
$\boldsymbol{m_{\widetilde{\mathrm{D}}^\mathrm{c}}^2}$, and $\boldsymbol{m_{\widetilde{\mathrm{E}}^\mathrm{c}}^2}$ is a $3\times3$
hermitian matrix in generation space and corresponds to the different sfermion
mass terms. Finally, $m_{\PH_\PQu}^2$ and $m_{\PH_\PQd}^2$ are squared-mass parameters for the Higgs bosons and $b$ is a cross term in the Higgs potential.
...
At the LHC, previous searches from ATLAS~\cite{Aad:2013wta,Aad:2014lra,Aad:2014pda,Aad:2014bva,Aad:2014qaa}
and CMS~\cite{1LepMVA,SUS12024,Chatrchyan:2014lfa,Chatrchyan:2013iqa,Chatrchyan:2013fea}
at $\sqrt{s}=8\TeV$ excluded top squarks up to $700\GeV$, and gluinos up
to $1.4\TeV$. In each case, search results were interpreted within
simplified models with only one SUSY particle decay mode considered at
a time. Fig.~\ref{fig:SUSY2013Summary} summarizes the state of SUSY
mass exclusions for the top squark and the gluino prior to the work described in this thesis\footnote{The SUSY exclusion
  summaries from CMS show a preliminary version of the results
  presented in this thesis labeled ``SUS-13-004 (Razor)''}. 

\begin{figure}[!htb] \centering
\includegraphics[width=0.49\textwidth]{figs/summary/ATLAS_SUSY_Stop_tLSP_201308.pdf}
\includegraphics[width=0.49\textwidth]{figs/summary/SUSY2013T2tt.pdf}\\
\includegraphics[width=0.49\textwidth]{figs/summary/ATLAS_SUSY_Gtt_201306.pdf}
\includegraphics[width=0.49\textwidth]{figs/summary/SUSY2013T1tttt.pdf}
\caption{ATLAS (left) and CMS (right) 8\TeV limits presented
  during summer 2013 for top squark (top) and gluino (bottom) pair production.
\label{fig:SUSY2013Summary}}
\end{figure}

...
tex->SetNDC();
   ...
   TLegendEntry *entry=leg->AddEntry("grae1","Data Control Region","pel");
   ...
   TLegendEntry *entry2=leg->AddEntry("grae","QCD MC Simulation","pel");
   ...
   TLegendEntry *entry3=leg->AddEntry("fxn_plus_err","Functional Form Model","lf");

   ci = TColor::GetColor("#0099ff");
   entry3->SetFillColor(ci);
   entry3->SetFillStyle(3254);

   ci = TColor::GetColor("#0000cc");
   entry3->SetLineColor(ci);
...
tex->SetNDC();
   ...
   c->cd();
   c->SetSelected(c);
   c->Print("npf_vs_mr_razor_fit.pdf");
}

...
seen in scouting data and more investigation is warranted.
...
is a wavelength-dependent loss of crystal transparency but without
changes to the scintillation mechanism~\cite{Adzic:2009aa}. Figure
~\ref{fig:ECALLaserHistory} demonstrates the loss in response over
time from the start of 2011.
...
%for. 
In order to measure and correct for response change during LHC
operation, the ECAL is equipped with a laser light monitoring
system~\cite{Anfreville:2007zz,Zhang:2005ip}. 
...
Relative energy response variation for EB (top) and EE (bottom)
...
after (green) LM corrections. Middle: Response stability during the
...
of the average LM corrections. Right: Distribution of the projected
relative energy scales.}
\end{figure}

These corrections are also applied and updated once-per-week at the HLT level for
data taking to prevent a worsening of the $e/\Pgg$ HLT resolution which would result in a severe rise in the
trigger rates of $e/\Pgg$ HLT paths. 
...
\caption{(Top) HLT super cluster energy relative to RECO super cluster
  energy for the ECAL barrel during 2015 Week 42. (Bottom) HLT super cluster energy relative to RECO super cluster
  energy for the ECAL endcaps during 2015 Week 41. The blue data points represent the
  reconstructed energy using the week-old laser corrections, whie the
  solid black histogram represents the reconstructed energy with the
  current laser correction.\label{fig:ECALHLT}}
\end{figure}

...
France and Switzerland. In 2016, the LHC collided protons at a center-of-mass energy of 13\TeV and instantaneous
luminosity exceeding the design value of $10^{34} \unit{cm}^{-2} \unit{s}^{-1}$.
\begin{table*}
\centering
...
Peak luminosity in IP1 and IP5 [cm$^{-1}$s$^{-1}$] &$1.0\times10^{34}$& $7.7\times10^{33}$ & $5.2\times10^{33}$& $1.2\times10^{34}$  \\
Max. mean number of events per bunch crossing& 19 & 40 & 17 & 40\\\hline\hline
\end{tabular}
}
\end{table*}
...
with the interaction point are discarded before entering the downstream
...
The $\Pgh$ and $\pi^0$ meson data are used to validate the laser corrections for prompt reconstruction. The events are
...
pairs in the mass range of the $\Pgh$ or $\pi^0$ mesons. The fit comprises a
...
and the relative value of the fitted $\pi^0$ mass versus time in EB for
a period of 8 hours. The plot shows the data before (red points) and
...
pb$^{-1}$. These events are collected by CMS with a dedicated trigger at a
rate of 8 (2.5) \unit{kHz} in the barrel (endcap), allowing to save only the
minimal information of the events, in particular energy deposits in
the ECAL crystals surrounding a possible $\pi^0$ candidate. These events
are used as prompt feedback to monitor the effectiveness of the laser
monitoring calibration and to intercalibrate the energy of ECAL
crystals. (Right) The stability of the relative energy scale measured from the invariant mass distribution of
...
is plotted as a function of time, over a period of 8 hours during an
LHC fill, for data recorded on May 28, 2016 during LHC fill 4958. The
plot shows the data with (green points) and without (red points) light
monitoring (LM) corrections applied. The right-hand panel shows the
...
relative response and are shown in Fig.~\ref{fig:wenuEB} for 2012 and 2015, as
a function of time. The data are shown before (red open circles) and after
(green filled circles) laser corrections to the ECAL channel response are
...
%and 0.28\% in the endcaps. This method does not require a
